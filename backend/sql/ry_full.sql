-- ==================================================================
-- 动物救助站宠物领养管理系统 - 全量安装脚本
-- 适用：全新数据库安装（可一键执行）
-- 执行：mysql -uroot -p < backend/sql/ry_full.sql
--
-- 内容结构：
--   第一部分 创建数据库 + 若依平台基础表与初始化数据(system/gen)
--   第二部分 定时任务表 (QRTZ)
--   第三部分 业务表与菜单 (tb_pet / tb_adoption_request /
--            tb_pet_health 及防重复申请触发器/唯一索引、菜单 2000~2045)
--   第四部分 演示数据（7 只宠物 + 健康记录 + 领养申请示例）
--
-- 重要说明：本脚本含 DROP + 重建表操作，仅可用于全新数据库，
--           已有业务数据的库请勿直接执行（会清空数据）。
--           默认数据库：ry_pet_adoption（utf8mb4）
-- ==================================================================

SET NAMES utf8mb4;

CREATE DATABASE IF NOT EXISTS `ry_pet_adoption` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;

USE `ry_pet_adoption`;

-- ==================================================================
-- 第一部分：若依平台基础表与初始化数据（sys_* / gen_*）
-- ==================================================================
-- ----------------------------
-- 1、部门表
-- ----------------------------
...
-- 第三部分：业务表与菜单
--   1. tb_pet               宠物信息表
--   2. tb_adoption_request  领养申请表（含"同一宠物同一时刻最多一条
--                          待审核申请"触发器 + 唯一索引 uk_pending_pet）
--   3. tb_pet_health        宠物健康记录表
--   4. 菜单数据(2000~2045) 与普通角色(role_id=2)授权
-- ==================================================================
-- ----------------------------
-- 1、宠物信息表
-- ----------------------------
...
-- ==================================================================
-- 第四部分：演示数据
--   宠物图片为后端静态资源 static/images/pets/*.jpg
--   状态取值与代码常量保持一致（可领养 / 已领养、pending / pass 等）
-- ==================================================================

-- 1、宠物演示数据（7 只，对应 7 张内置图片）
insert into tb_pet (pet_id, name, breed, age, gender, weight, status, image_url, description, rescue_date, create_by, create_time) values
(100, '旺财', '中华田园犬', 24, '1', 12.50, '可领养', '/images/pets/wangcai.jpg', '亲人温顺的大型犬，适合有院子的家庭。', '2025-06-18', 'admin', sysdate());
insert into tb_pet (pet_id, name, breed, age, gender, weight, status, image_url, description, rescue_date, create_by, create_time) values
(101, '哈吉米', '橘猫', 6, '1', 4.20, '可领养', '/images/pets/hajimi.jpg', '小吃货一枚，活泼粘人。', '2025-07-02', 'admin', sysdate());
insert into tb_pet (pet_id, name, breed, age, gender, weight, status, image_url, description, rescue_date, create_by, create_time) values
(102, '小白', '中华田园猫', 18, '0', 3.60, '可领养', '/images/pets/xiaobai.jpg', '安静乖巧，已绝育。', '2025-05-20', 'admin', sysdate());
insert into tb_pet (pet_id, name, breed, age, gender, weight, status, image_url, description, rescue_date, create_by, create_time) values
(103, '小七', '英国短毛猫', 8, '1', 4.80, '可领养', '/images/pets/xiaoqi.jpg', '圆脸萌宠，性格温和。', '2025-07-30', 'admin', sysdate());
insert into tb_pet (pet_id, name, breed, age, gender, weight, status, image_url, description, rescue_date, create_by, create_time) values
(104, '小黑', '中华田园犬', 36, '0', 9.00, '已领养', '/images/pets/xiaohei.jpg', '已找到温暖的新家。', '2025-04-12', 'admin', sysdate());
insert into tb_pet (pet_id, name, breed, age, gender, weight, status, image_url, description, rescue_date, create_by, create_time) values
(105, '胡子', '狸花猫', 14, '1', 4.30, '可领养', '/images/pets/huzi.jpg', '起初警惕，熟悉后很粘人。', '2025-06-05', 'admin', sysdate());
insert into tb_pet (pet_id, name, breed, age, gender, weight, status, image_url, description, rescue_date, create_by, create_time) values
(106, '毛弟', '柯基犬', 10, '0', 7.50, '可领养', '/images/pets/maodie.jpg', '短腿活泼，亲人爱玩。', '2025-08-01', 'admin', sysdate());

-- 2、宠物健康记录演示数据（对应宠物 100 旺财 / 102 小白）
insert into tb_pet_health (health_id, pet_id, record_date, vaccine_name, is_sterilized, health_status, description, create_by, create_time) values
(100, 100, '2025-08-20', '狂犬疫苗', 0, 'HEALTHY', '状态良好，进食正常。', 'admin', sysdate());
insert into tb_pet_health (health_id, pet_id, record_date, vaccine_name, is_sterilized, health_status, description, create_by, create_time) values
(101, 102, '2025-08-15', '猫三联', 1, 'GOOD', '已完成绝育，恢复良好。', 'admin', sysdate());

-- 3、领养申请演示数据
--    100 小黑(已领养) 的领养申请：审核通过(pass)，配套成功记录
--    101 旺财 的领养申请：待审核(pending)，供"我的申请"页面展示
insert into tb_adoption_request (request_id, pet_id, user_id, reason, status, review_remark, review_time, review_by, create_time) values
(100, 104, 2, '希望给小黑一个安稳的家，家里有院子。', 'pass', '审核通过，已办理领养。', sysdate(), 'admin', sysdate());
insert into tb_adoption_request (request_id, pet_id, user_id, reason, status, review_remark, review_time, review_by, create_time) values
(101, 100, 2, '家里院子宽敞，适合大型犬活动。', 'pending', '', null, '', sysdate());

-- 4、领养成功记录演示数据（与领养申请 100 / 宠物 104 对应）
insert into tb_adoption_success (success_id, request_id, pet_id, user_id, adopt_time, follow_up_date, status, remark, create_time) values
(100, 100, 104, 2, sysdate(), '2025-10-22', 'success', '小黑已顺利入住新家，回访一切正常。', sysdate());
